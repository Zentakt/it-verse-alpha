...
import { Scan, Keyboard, Camera, RefreshCw, X, CheckCircle2, Zap, AlertTriangle, Brain, ArrowRight, ShieldCheck, Trophy } from 'lucide-react';
...
const QRScanner: React.FC<QRScannerProps> = ({ currentTeam, challenges }) => {
    ...
    const handleGameComplete = (success: boolean) => {
        if (success && activeChallenge) {
            const newSolved = [...solvedChallengeIds, activeChallenge.id];
            // Ensure unique
            const uniqueSolved = Array.from(new Set(newSolved));
            setSolvedChallengeIds(uniqueSolved);

            setChallengeSolved(true);
            setShowGameOverlay(false);

            const gameChallenges = challenges.filter(c => c.gameType && c.gameType !== 'none');
            if (uniqueSolved.length === gameChallenges.length) {
                setAllCleared(true);
            }
        } else {
    ...
    const resetScanner = () => {
        ...
        setUserAnswer('');
        setManualCode('');
        setAllCleared(false); // Reset allCleared state
        setSolvedChallengeIds([]); // Reset solved challenges
        initBarcodeDetector();
        ...
        <div ref={containerRef} className="w-full h-full relative bg-black overflow-hidden flex flex-col" style={{ minHeight: 'calc(100vh - 80px)' }}>
            ...
            {allCleared && (
                <div className="absolute inset-0 z-50 bg-black/95 flex flex-col items-center justify-center p-8 text-center animate-in zoom-in duration-500">
                    <div className="w-32 h-32 bg-yellow-500/20 rounded-full flex items-center justify-center mb-6 border border-yellow-500 shadow-[0_0_50px_#eab308]">
                        <Trophy size={64} className="text-yellow-400 drop-shadow-lg" />
                    </div>
                    <h1 className="text-4xl md:text-5xl font-black font-cyber text-white tracking-widest mb-2 text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-white to-yellow-400 animate-shimmer">
                        SYSTEM OVERRIDE
                    </h1>
                    <p className="text-yellow-500 font-mono text-sm tracking-[0.3em] mb-8">ALL SECURITY PROTOCOLS NEUTRALIZED</p>

                    <div className="space-y-2 w-full max-w-md bg-white/5 p-4 rounded-lg border border-white/10">
                        {challenges.filter(c => c.gameType && c.gameType !== 'none').map(c => (
                            <div key={c.id} className="flex justify-between items-center text-xs font-mono text-gray-400 border-b border-white/5 last:border-0 py-2">
                                <span>{c.title}</span>
                                <span className="text-green-400 flex items-center gap-1"><CheckCircle2 size={12} /> BYPASSED</span>
                            </div>
                        ))}
                    </div>

                    <button
                        onClick={() => { setAllCleared(false); setSolvedChallengeIds([]); setActiveChallenge(null); setScannedResult(null); setChallengeSolved(false); startCamera(); }}
                        className="mt-8 px-8 py-3 bg-white text-black font-bold font-cyber tracking-widest rounded hover:scale-105 transition-transform"
                    >
                        REBOOT SYSTEM
                    </button>
                </div>
